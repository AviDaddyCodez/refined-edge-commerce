    ...
    <Link to={`/products/${id}`}>
      ...
      >
        <div className="relative overflow-hidden rounded-xl mb-4 aspect-[3/4] bg-gray-100">
          ...
          />
          <div
            className={`absolute inset-0 bg-gradient-to-b from-transparent to-black/20 transition-opacity duration-300 ${
              isHovered ? "opacity-100" : "opacity-0"
            }`}
          ></div>
          <button
            onClick={handleAddToCart}
            className={`absolute bottom-4 right-4 bg-white hover:bg-gray-50 rounded-full p-3 shadow-md transition-all duration-300 ${
          ...
          >
            <div className="py-3 px-4 bg-black/60 backdrop-blur-sm text-white text-sm font-medium">
              Quick View
            </div>
          </div>
        </div>
